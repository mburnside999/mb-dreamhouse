...
export class PropertyService {
  ...
  findAll() {
    return this.http
      .get("/property")
      .map((response) => response.json().map(prettifyProperty));
  }

  findById(id) {
    return this.http
      .get("/property/" + id)
      .map((response) => prettifyProperty(response.json()));
  }

  getFavorites() {
    return this.http
      .get("/favorite")
      .map((response) => response.json().map(prettifyFavorite));
  }
  ...
  favorite(property) {
    ...
    return this.http.post(
      "/favorite",
      JSON.stringify({ property__c: property.id }),
      { headers: headers }
    );
  }
