...
var connectionString =
  process.env.DATABASE_URL || "postgres://localhost:5432/dreamhouse";
...
client.query("SELECT * FROM salesforce.broker__c", function (error, data) {
  if (error !== null) {
    ...
    favoriteTable = schema + "favourite__c";
...
app.post("/favorite", function (req, res) {
  client.query(
    "INSERT INTO " + favoriteTable + " (property__c) VALUES ($1)",
    [req.body.property__c],
    function (error, data) {
      res.json(data);
    }
  );
...
app.delete("/favorite/:sfid", function (req, res) {
  client.query(
    "DELETE FROM " + favoriteTable + " WHERE sfid = $1",
    [req.params.sfid],
    function (error, data) {
      res.json(data);
    }
...
app.get("/broker/:sfid", function (req, res) {
  client.query(
    "SELECT * FROM " + brokerTable + " WHERE sfid = $1",
    [req.params.sfid],
    function (error, data) {
      res.json(data.rows[0]);
    }
